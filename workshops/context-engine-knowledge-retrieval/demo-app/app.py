...
if _live_enabled:
    try:
        import context_lab  # noqa: F401 — imported for side effects; live routes below

        AI_INDEX = os.environ.get("CE_AI_INDEX", "ai-index-idx-precision-corpus")

        SYSTEM_PROMPT_BASE = (
            "You are a customer success AI assistant for Precision Labs. "
            "You have access to three data sources: precision-crm-accounts, "
            "precision-support-tickets, and precision-kb-articles. "
        ...
        def _configure_once():
            global _configured, _current_model
            if _configured:
                return
            context_lab.configure(
                es_url=os.environ.get("ES_URL", ""),
                api_key=os.environ.get("ES_API_KEY", ""),
        ...
        def ask():
            ...
            try:
                ...
                tools = [context_lab.ESQL_TOOL, context_lab.MAPPING_TOOL]
                result = context_lab.run_agent(
